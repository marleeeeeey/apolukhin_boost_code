...
{
    // ...
    BOOST_STATIC_ASSERT(sizeof(val) != 1 && sizeof(val) != 4 && sizeof(val) != 8);
    return val;
}
...
int main()
{
    std::cout << ' ' << process(int(0)) << ' ' << process(double(1)) << ' ' << process(float(2)) << ' '
              << process(char(3)) << ' ' << process(unsigned(4)) << ' ' << process(short(5)) << std::endl;
}
