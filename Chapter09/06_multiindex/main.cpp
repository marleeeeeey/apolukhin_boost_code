...
void print(const Indexes& persons)
{
    ...
    typedef typename Indexes::template nth_index<IndexNo>::type::const_iterator const_iterator_t;

    for (const_iterator_t it = persons.template get<IndexNo>().begin(), iend = persons.template get<IndexNo>().end();
         it != iend; ++it)
    {
        const person& v = *it;
        std::cout << v.name_ << ", " << v.id_ << ", " << v.height_ << ", " << v.weight_ << '\n';
    }
...
void example_main()
{
    typedef boost::multi_index::multi_index_container<
        ...
        boost::multi_index::indexed_by<
            ...
            boost::multi_index::hashed_non_unique<boost::multi_index::member<person, std::size_t, &person::id_>>,

            // Height may not be unique, but must be sorted
            boost::multi_index::ordered_non_unique<boost::multi_index::member<person, unsigned int, &person::height_>>,

            // Weight may not be unique, but must be sorted
            boost::multi_index::ordered_non_unique<boost::multi_index::member<
                person, unsigned int, &person::weight_>>> // closing for `boost::multi_index::indexed_by<`
        >
        indexes_t;
    ...
    for (const_iterator_t it = persons.get<0>().begin(), iend = persons.get<0>().end(); it != iend; ++it)
    {
        const person& v = *it;
        // ...
        (void)v;
    ...
    typedef boost::multi_index::multi_index_container<
        ...
        boost::multi_index::indexed_by<
            ...
            boost::multi_index::ordered_unique<
                boost::multi_index::tag<tag::name>, boost::multi_index::member<person, std::string, &person::name_>>,

            // IDs are not unique, but we do not need then ordered
            boost::multi_index::hashed_non_unique<
                boost::multi_index::tag<tag::id>, boost::multi_index::member<person, std::size_t, &person::id_>>,
