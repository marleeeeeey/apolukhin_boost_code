...
int main()
{
    ...
    name_id.insert(name_id_type::value_type("Anton Polukhin", 3));
    ...
    for (left_const_iterator it = name_id.left.begin(); it != lend; ++it)
    {
    ...
    for (right_const_iterator it = name_id.right.begin(); it != rend; ++it)
    {
        std::cout << it->first << " <=> " << it->second << '\n';
    }
...
typedef boost::bimap<boost::bimaps::set_of<std::string>, boost::bimaps::multiset_of<std::size_t>> name_id_type;
...
typedef boost::bimap<boost::bimaps::unordered_set_of<std::string>, boost::bimaps::unordered_multiset_of<std::size_t>>
    hash_name_id_type;
