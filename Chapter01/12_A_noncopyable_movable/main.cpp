...
public:
    ...
    void swap(descriptor_owner& desc) { std::swap(descriptor_, desc.descriptor_); }

    ~descriptor_owner() { free(descriptor_); }
};

#include <string.h>
descriptor_owner::descriptor_owner(const char* param) : descriptor_(strdup(param))
{}
...
public:
    ...
    descriptor_owner1(descriptor_owner1&& param) : descriptor_(param.descriptor_) { param.descriptor_ = nullptr; }
...
descriptor_owner1::descriptor_owner1(const char* param) : descriptor_(strdup(param))
{}
...
public:
    ...
    ~descriptor_owner_movable() BOOST_NOEXCEPT { clear(); }
};
