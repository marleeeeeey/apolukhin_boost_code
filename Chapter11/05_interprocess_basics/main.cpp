...
int main()
{
    boost::interprocess::managed_shared_memory segment(boost::interprocess::open_or_create, "shm1-cache", 1024);

    atomic_t& atomic = *segment.find_or_construct<atomic_t> // 1
                        ("shm1-counter") // 2
                        (0) // 3
        ;

    std::cout << "I have index " << ++atomic << ". Press any key...\n";
    std::cin.get();
