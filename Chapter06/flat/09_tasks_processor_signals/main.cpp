...
public:
    explicit timer_task(std::unique_ptr<boost::asio::deadline_timer> timer, const Functor& task_unwrapped)
...
struct connection_with_data : boost::noncopyable
{
    ...
    explicit connection_with_data(Executor executor) : socket(executor)
    {}
...
private:
    ...
    T task_unwrapped_;
public:
    explicit task_wrapped_with_connection(connection_ptr&& c, const T& f) : c_(std::move(c)), task_unwrapped_(f) {}
...
void async_read_data(connection_ptr&& c, const Functor& f, std::size_t at_least_bytes)
{
    ...
    boost::asio::async_read(
        s, boost::asio::buffer(p, d.size()), task_wrapped_with_connection<Functor>(std::move(c), f));
}
...
void async_read_data_at_least(connection_ptr&& c, const Functor& f, std::size_t at_least_bytes, std::size_t at_most)
{
    ...
    boost::asio::async_read(
        s, boost::asio::buffer(p, at_most), boost::asio::transfer_at_least(at_least_bytes),
...
public:
    static connection_ptr create_connection(const char* addr, unsigned short port_num)
    {
        ...
        c->socket.connect(boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::from_string(addr), port_num));
...
private:
    struct tcp_listener
    {
        ...
        template <class Functor>
        tcp_listener(boost::asio::io_service& io_service, unsigned short port, const Functor& task_unwrapped)
...
private:
    static void handle_signals(const boost::system::error_code& error, int signal_number)
    {
        ...
        else
        {
            ...
            detail::make_task_wrapped([h, signal_number]() { h(signal_number); })(); // make and run task_wrapped
        }
    }
public:
    ...
    template <class Func>
    static void register_signals_handler(const Func& f, std::initializer_list<int> signals_to_wait)
    {
        ...
        std::for_each(signals_to_wait.begin(), signals_to_wait.end(), [&sigs](int signal) { sigs.add(signal); });
...
int main()
{
    tasks_processor::register_signals_handler(&accept_3_signals_and_stop, {SIGINT, SIGSEGV});
