...
private:
    answer_t default_;
public:
    postprocessor() : default_(read_defaults()) {}
...
public:
    postprocessor() : default_flag_(BOOST_ONCE_INIT), default_() {}
    ...
    answer_t act(const std::string& in) const
    {
        ...
        {
            // Extremely rare condition.
            boost::call_once(default_flag_, [this]() { this->default_ = read_defaults(); });
            return default_;
        }
...
void once_printer(int i)
{
    ...
    boost::call_once(
        flag, [](int v) { std::cout << "Print once " << v << '\n'; },
...
std::vector<std::string> read_defaults()
{
    std::vector<std::string> ret{"Do", "not", "look", "at", "me"};
    return ret;
}
