...
date parse_date_time1(const std::string& s)
{
    ...
    const bool success = boost::spirit::qi::parse(
        ...
        ushort_[ref(res.year) = _1] >> char_('-') >> ushort_[ref(res.month) = _1] >> char_('-') >>
...
date parse_date_time2_cxx(const std::string& s)
{
    ...
    const auto y = [&res](unsigned short s)
    {
        res.year = s;
    };
    const auto m = [&res](unsigned short s)
    {
        res.month = s;
    };
    const auto d = [&res](unsigned short s)
    {
        res.day = s;
    };
    ...
    const char* first = s.data();
    const char* const end = first + s.size();
    const bool success = boost::spirit::qi::parse(first, end, u4_[y] >> char_('-') >> u2_[m] >> char_('-') >> u2_[d]);
